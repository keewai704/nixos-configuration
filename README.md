...
Pi 0.85.1 で前景・バックグラウンド実行を確認した 0.70.0 に固定します。
0.70.1 はバックグラウンド runner が Pi 0.85.1 にない `pi-ai.createInitialSystemMessage` を参照して
起動できないため使いません。本体・Codex conversion と合わせて互換性を検証してから更新します。
