...
変更された項目名だけを画面に知らせます。`/cache` でも直近の変更項目を確認できます。
変更回数と比較基準はセッションの読み込み・再読み込み時にリセットします。
...
開始承認後に Brave Origin を専用の一時プロフィールで自動起動し、デバッグ接続はループバックの一時ポートへ限定します。
ブラウザーの手動起動や接続許可は不要です。デスクトップセッションではウィンドウを表示し、
表示環境がない場合は headless で起動します。Pi Web でもユーザーセッションの表示環境を引き継ぎます。
...
普段の Brave のプロフィール・タブ・ログイン状態は使用しません。
一時プロフィールによる分離は、ネットワークや OS 権限のサンドボックスではありません。

既定は最大12回の判断・120秒、上限は30回・300秒です。時間にはブラウザー起動と操作承認待ちも含みます。
Pi のキャンセルとセッション終了で停止します。実行ごとの一時的な systemd ユーザースコープで
専用ブラウザーと接続デーモンをまとめて管理し、終了後に停止と一時プロフィールの削除を確認します。
Pi が異常終了してもスコープの制限時間で停止しますが、その場合の一時ファイル削除は保証しません。
停止処理には数秒の猶予があります。同じ利用者の Jev ツールは排他実行します。
CUA でこの専用ブラウザーを同時操作しないでください。
...
専用ブラウザー全体の停止とプロフィール削除は別の `browser_cleanup` で確認できます。
