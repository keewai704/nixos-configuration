...
Inspect the role's actual prerequisites. In configured Pi, use native Agent's
code-reviewer for correctness or quality-reviewer for maintainability. Supply a
readable complete diff and paths: these roles have no shell. Retrieve
get_subagent_result and close the verified delivery with manage_subagents.
Do not create a repository or commit solely to run a review. If independent
