...
| モデル・自律的な完遂・確認条件 | [Codex追加指示](../modules/codex.nix) | 全プロジェクト共通の `developer_instructions` として定義する |
| 人間が読むコードの可読性 | Codex追加指示 | 明確な名前、素直な制御フロー、責務に合う構成を必須とし、判断理由と制約を必要に応じてコメントする |
...
- **可読性:** コードと設定は人が読み、レビューし、保守するものとして書く。
  短さだけを目的とした圧縮や技巧的な省略を避ける。
