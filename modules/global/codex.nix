...
let
  # Keep Codex's shared defaults in the system layer so its user config
      ...
      let
        ...
        codexWrapper = pkgs.writeShellApplication {
          ...
          text = ''
            ...
            # Serialize only the config update. Do not let a long-running
            # Codex process retain the lock or concurrent clients cannot run.
