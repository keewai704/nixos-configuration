...
const HELP: &str = r#"Siora — Apple Music TUI
...
fn main() -> Result<()> {
    ...
    let data_directory = std::env::var_os("ALAC_ROOM_DATA_DIR")
        .map(PathBuf::from)
        .or_else(|| {
    ...
    let store = LibraryStore::open(data_directory.join("apple-music.json"))?;
    tui::run(store, data_directory.join("downloads"), no_auth, url)
}
